...
public class NewParser implements Serializable {
  ...
  private void parseHeaderPerson(String header) throws BadEntryException {
    ...
    switch (components[0]) {
    case "FUNCIONÁRIO":
      _person = new Employee(components[3], phone, id);
      break;

    case "DOCENTE":
      _person = new Teacher(components[3], phone, id);
      break;

    case "ALUNO":
      _person = new Student(components[3], phone, id, false);
      break;

    case "DELEGADO":
      _person = new Student(components[3], phone, id, true);
      break;
